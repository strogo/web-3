...
def buildGithubCommentVisualDiff(ctx, suite, alternateSuiteName, runningOnOCIS):
	...
	return [{
		...
		'commands': [
			...
			'for f in *.png; do echo \'!\'"[$f](https://minio.owncloud.com/web/screenshots/${DRONE_BUILD_NUMBER}/diff/%s/$f)" >> /var/www/owncloud/web/comments.file; done' % backend,
			'cd ../../latest',
			'cd %s' % backend,
			'echo "Actual Image: </br>" >> /var/www/owncloud/web/comments.file',
			'for f in *.png; do echo \'!\'"[$f](https://minio.owncloud.com/web/screenshots/${DRONE_BUILD_NUMBER}/latest/%s/$f)" >> /var/www/owncloud/web/comments.file; done' % backend,
